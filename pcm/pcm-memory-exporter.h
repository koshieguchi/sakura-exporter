...
void printSocketChannelBW(PCM *, memdata_t *md, uint32 no_columns, uint32 skt)
{
  for (uint32 channel = 0; channel < max_imc_channels; ++channel)
  {
    ...
    if (bad_channels == no_columns)
    { // the channel is missing on all sockets in the row
      continue;
    }
    for (uint32 i = skt; i < (skt + no_columns); ++i)
    {
...
void printSocketBWFooter(PCM *m, uint32 no_columns, uint32 skt, const memdata_t *md)
{
  ...
  if (md->metrics == PartialWrites && m->getCPUFamilyModel() != PCM::SRF && m->getCPUFamilyModel() != PCM::GNR)
  {
    for (uint32 i = skt; i < (skt + no_columns); ++i)
    {
  ...
  for (uint32 i = skt; i < (skt + no_columns); ++i)
  {
    cout << "|-- SKT " << setw(2) << i << " Memory (MB/s): " << setw(11) << right << (md->iMC_Rd_socket[i] + md->iMC_Wr_socket[i] + md->iMC_PMM_Rd_socket[i] + md->iMC_PMM_Wr_socket[i]) << " --|";
  }
  cout << "\n";
  for (uint32 i = skt; i < (no_columns + skt); ++i)
  {
...
void display_bandwidth(PCM *m, memdata_t *md, const uint32 no_columns, const bool show_channel_output, const bool print_update, const float CXL_Read_BW)
{
  ...
  }
  {
    ...
    {
      cout << "\
            \r|--            System DRAM Read Throughput(MB/s):"
           << setw(14) << sysReadDRAM << "                --|\n\
            \r|--           System DRAM Write Throughput(MB/s):"
           << setw(14) << sysWriteDRAM << "                --|\n\
            \r|--             System PMM Read Throughput(MB/s):"
           << setw(14) << sysReadPMM << "                --|\n\
            \r|--            System PMM Write Throughput(MB/s):"
           << setw(14) << sysWritePMM << "                --|\n";
    }
    if (SPR_CXL)
    {
      cout << "\
            \r|--             System CXL Read Throughput(MB/s):"
           << setw(14) << CXL_Read_BW << "                --|\n";
    }
    cout << "\
            \r|--                 System Read Throughput(MB/s):"
         << setw(14) << sysReadDRAM + sysReadPMM << "                --|\n\
            \r|--                System Write Throughput(MB/s):"
         << setw(14) << sysWriteDRAM + sysWritePMM << "                --|\n\
            \r|--               System Memory Throughput(MB/s):"
         << setw(14) << sysReadDRAM + sysReadPMM + sysWriteDRAM + sysWritePMM << "                --|\n\
            \r|---------------------------------------||---------------------------------------|\n";
  }
}
  ...
  for (uint32 skt = 0; skt < numSockets; ++skt)
  {
    ...
    if (show_channel_output)
    {
      for (uint64 channel = 0; channel < max_imc_channels; ++channel)
      {
        ...
        choose(outputType, [printSKT]()
               { printSKT(2); }, [&channel]()
               { cout << "Ch" << channel << "Read,"
                      << "Ch" << channel << "Write,"; }, [&md, &skt, &channel, &invalid_data]()
               {
                           if (invalid_data)
                               cout << ",,";
                           else
        ...
        if (md->metrics == Pmem)
        {
          choose(outputType, [printSKT]()
                 ...
                 { cout << "Ch" << channel << "PMM_Read,"
                        << "Ch" << channel << "PMM_Write,"; }, [&skt, &md, &channel, &invalid_data]()
                 {
                               if (invalid_data)
                                   cout << ",,";
                               else
    ...
    choose(outputType, [printSKT]()
           { printSKT(2); }, []()
           { cout << "Mem Read (MB/s),Mem Write (MB/s),"; }, [&md, &skt]()
    ...
    if (anyPmem(md->metrics))
    {
      choose(outputType, [printSKT]()
             { printSKT(2); }, []()
             { cout << "PMM_Read (MB/s), PMM_Write (MB/s),"; }, [&md, &skt]()
    ...
    if (md->metrics == Pmem && md->M2M_NM_read_hit_rate_supported)
    {
      for (uint32 c = 0; c < max_imc_controllers; ++c)
      {
        choose(outputType, [printSKT]()
               { printSKT(); }, [c]()
               { cout << "iMC" << c << " NM read hit rate,"; }, [&md, &skt, c]()
    ...
    if (md->metrics == PmemMixedMode)
    {
      choose(outputType, [printSKT]()
             { printSKT(3); }, []()
             { cout << "PMM_AD (MB/s), PMM_MM (MB/s), PMM_MM_Bw/DRAM_Bw,"; }, [&md, &skt]()
    ...
    {
      if (md->metrics == PartialWrites && m->getCPUFamilyModel() != PCM::GNR && m->getCPUFamilyModel() != PCM::SRF)
      {
        choose(outputType, [printSKT]()
               { printSKT(); }, []()
               { cout << "P. Write (T/s),"; }, [&md, &skt]()
        ...
        for (uint64 channel = 0; channel < max_edc_channels; ++channel)
        {
          ...
          choose(outputType, [printSKT]()
                 ...
                 { cout << "EDC_Ch" << channel << "Read,"
                        << "EDC_Ch" << channel << "Write,"; }, [&md, &skt, &channel]()
    ...
    {
      choose(outputType, [printSKT, &md]()
             { printSKT((md->BHS) ? 4 : 2); }, [&port, &md]()
             {
                    if (md->BHS)
                    {
      ...
      for (uint32 channel = 0; channel < max_imc_channels; ++channel)
      {
        ...
        else if (metrics == PmemMemoryMode)
        {
          ...
          md.MemoryMode_Hit_socket[skt] += toRate(memoryModeHits);
            ...
            cpu_family_model != PCM::GNR && cpu_family_model != PCM::SRF)
        {
...
public:
  CHAEventCollector(const double delay_, const char *sysCmd_, const MainLoop &mainLoop_, PCM *m) : sysCmd(sysCmd_),
                                                                                                   mainLoop(mainLoop_),
                                                                                                   pcm(m)
  {
    assert(pcm);
